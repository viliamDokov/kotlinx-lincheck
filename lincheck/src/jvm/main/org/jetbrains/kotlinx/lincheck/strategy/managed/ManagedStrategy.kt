...
internal abstract class ManagedStrategy(
    ...
    protected val memorySnapshot = SnapshotTracker().apply {
        if (runner is LambdaRunner<*>) {
           ...
           runner.capturedObjects().forEach(::trackObjectAsRoot)
        }
    }
    ...
    override fun afterThreadRunException(
        ...
        if (isLincheckInternalException(exception)) {
            ...
            disableAnalysis()
            Logger.info(exception) { "Exception was thrown in user thread \"Thread-$currentThreadId\":" }
    ...
    override fun onActorFinish(iThread: Int) = runInsideIgnoredSection {
        ...
        if (actorStartTracePoint != null) {
            ...
            val actorFinishTracePoint = MethodReturnTracePoint(context, getNextEventId(), actorStartTracePoint)
    ...
    override fun onAwaitLoopPath(
        ...
        if (currentExecutionPart !== VALIDATION && !threadScheduler.isAborted(threadId)) {
            val decision = loopDetector.onAwaitLoopPath(threadId, codeLocation, loopId)
            processLoopDetectorDecision(decision, threadId, loopId, codeLocation)
        }
    }
    ...
    private fun addBeforeMethodCallTracePoint(
        ...
        if (isScenarioThread(threadId) && isResumptionMethodCall(threadId, className, methodName, methodParams, atomicMethodDescriptor)) {
            ...
            suspendedMethodStack.subList(0, elementIndex).forEach { element ->
                val returnTracePoint = MethodReturnTracePoint(context, getNextEventId(), element.tracePoint)
                traceCollector?.addTracePointInternal(returnTracePoint)
            }
    ...
    private fun afterMethodCall(iThread: Int, tracePoint: MethodCallTracePoint) {
        ...
        popShadowStackFrame()
        callStackTrace.removeLast()
        traceCollector?.addTracePointInternal(MethodReturnTracePoint(context, eventId, tracePoint))
    }
